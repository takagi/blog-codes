...
ps0 :: [Particle]
ps0 = [ Particle { pos  = Vector3 x y z,
                   vel  = Vector3 0 0 0,
                   mass = pmass,
                   rho  = 0,
                   prs  = 0 }
...
neighbor_idx (Vector3 x y z) = (toIx x, toIx y, toIx z)
  where toIx :: Scalar -> Int
        toIx x = floor(x*simscale/sph_h)
        
neighbor_map :: [Particle] -> NeighborMap
neighbor_map ps = let ix_min   = let (ix,iy,iz) = neighbor_idx box_min
                                 in (ix-1,iy-1,iz-1)
  ...
  where _neighbor_list :: [Particle] -> NeighborList -> NeighborList
...
insert_neighbor p@(Particle {pos=r}) nbr_map =
  ...
  where _insert_neighbor :: NeighborIdx -> Particle -> NeighborList
                                                    -> NeighborList
...
neighbor nbr_map r = let (_ix,_iy,_iz) = neighbor_idx r
...
advance ps nbr_map = map _advance ps
  where _advance p@(Particle r v m rho prs) =
...
force ps p = pressure_term ps p
           + viscosity_term ps p

pressure_term :: [Particle] -> Particle -> Vector3
pressure_term ps i = -(sum $ map exp $ filter (/=i) ps)
  where exp :: Particle -> Vector3
...
viscosity_term ps i = visc * (sum $ map exp $ filter (/=i) ps)
  where exp :: Particle -> Vector3
...
boundary_condition p@Particle {pos=r,vel=v} a = x_wall_max $ x_wall_min
                                              $ y_wall_max $ y_wall_min
                                              $ z_wall_max $ z_wall_min
                                              $ accel_limit a
  where x_wall_max = wall ((v3x box_max)-(v3x r)) (Vector3 (-1) 0 0)
...
output_povs ps i = do putStrLn ("processing "++ (file_name i)++" ...")
                      write ps i
  where write :: [Particle] -> Int -> IO()
...
output_csvs ps i = do putStrLn ("processing "++ (file_name i)++" ...")
                      write ps i
  where write :: [Particle] -> Int -> IO()
...
rjust width s = replicate (width - (length $ show s)) '0' ++ (show s)
rjust8 = rjust 8
